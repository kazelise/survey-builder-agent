...
**Update (2026-07-03):** the eval suite has since grown to 39 cases (4 new
`handbook_rag` cases). `evals/EVAL_REPORT.md` was a stale, un-regenerated
duplicate of this run and has been removed; `evals/REPORT.md` (the default
`--report` path, regenerated by every `uv run python -m evals.runner`) is
the current mock-mode report and reads **39/39 (100%)**, with `handbook_rag`
(4/4) added to the category breakdown.
...
- 39/39 (100%) automated mock-mode evals passing across single-step,
  multi-step, ambiguous-instruction, error-recovery, refuse-overreach, and
  handbook-RAG categories (35/35 at the time of this verification run; the
  suite grew by 4 handbook_rag cases afterward — see `evals/REPORT.md` for
  the current figure).
